...
if c.is_open():
    # Issue Command Data
    result = c.write_multiple_registers(0, [12, 0x0400, 3])
    # Enter preset amount here (float)
    result = c.write_float(3, [3000.0])
    
    # Additive Mask (0xFFFF -> recipe-driven)
    result = c.write_single_register(5, 0xFFFF)
    result = c.write_single_register(6, 0xFFFF)
